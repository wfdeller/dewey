...
const displayValue = (value: string | number | boolean | null | undefined, formatter?: (v: string | number | boolean) => string): string => {
  ...
  const openEditModal = (type: EditModalType) => {
    ...
    if (type === 'contact') {
      ...
      Object.assign(formValues, {
        ...
        voter_registration_date: contact.voter_registration_date ? dayjs(contact.voter_registration_date) : null,
      });
    } else if (type === 'status') {
      Object.assign(formValues, {
    ...
    <div>
      ...
      {!contact.is_active && (
        <Alert
          ...
          description={`This contact is marked as inactive${contact.inactive_reason ? `: ${formatLabel(contact.inactive_reason)}` : ''}`}
          type="warning"
          showIcon
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            {contact.name && (
              <Text type="secondary" style={{ display: 'block' }}>{contact.email}</Text>
            )}
          </div>
          <Popconfirm
      ...
      <Row gutter={24}>
        ...
        <Col xs={24} lg={8}>
          {/* Basic Info Card */}
          <Card
            title={<><UserOutlined /> Contact Information</>}
            style={{ marginBottom: 16 }}
            extra={<Button type="text" icon={<EditOutlined />} onClick={() => openEditModal('contact')}>Edit</Button>}
          >
            <Descriptions column={1} size="small">
              ...
              <Descriptions.Item label="Language">
                {displayValue(contact.preferred_language) === 'N/A' ? 'N/A' : contact.preferred_language?.toUpperCase()}
              </Descriptions.Item>
            </Descriptions>
          </Card>
          ...
          {/* Demographics Card */}
          <Card
            title={<><IdcardOutlined /> Demographics</>}
            style={{ marginBottom: 16 }}
            extra={<Button type="text" icon={<EditOutlined />} onClick={() => openEditModal('demographics')}>Edit</Button>}
          >
            <Descriptions column={1} size="small">
              <Descriptions.Item label="Gender">
                {formatLabel(contact.gender)}
              </Descriptions.Item>
          ...
          {/* Professional Card */}
          <Card
            title={<><BankOutlined /> Professional</>}
            style={{ marginBottom: 16 }}
            extra={<Button type="text" icon={<EditOutlined />} onClick={() => openEditModal('professional')}>Edit</Button>}
          >
            <Descriptions column={1} size="small">
              <Descriptions.Item label="Occupation">
                {displayValue(contact.occupation)}
              </Descriptions.Item>
              <Descriptions.Item label="Job Title">
                {displayValue(contact.job_title)}
              </Descriptions.Item>
              <Descriptions.Item label="Employer">
                {displayValue(contact.employer)}
              </Descriptions.Item>
              <Descriptions.Item label="Industry">
                {displayValue(contact.industry)}
              </Descriptions.Item>
            </Descriptions>
          </Card>
          ...
          <Card
            title={<><EnvironmentOutlined /> Location & District</>}
            style={{ marginBottom: 16 }}
            extra={<Button type="text" icon={<EditOutlined />} onClick={() => openEditModal('location')}>Edit</Button>}
          >
            <Descriptions column={1} size="small">
          ...
          <Card
            title={<><TeamOutlined /> Voter Information</>}
            style={{ marginBottom: 16 }}
            extra={<Button type="text" icon={<EditOutlined />} onClick={() => openEditModal('voter')}>Edit</Button>}
          >
            <Descriptions column={1} size="small">
              <Descriptions.Item label="Voter Status">
                <Tag color={
                  contact.voter_status === 'active' ? 'green' :
                  contact.voter_status === 'inactive' ? 'orange' :
                  contact.voter_status === 'unregistered' ? 'default' : 'default'
                }>
          ...
          <Card title="Activity Statistics" style={{ marginBottom: 16 }}>
            ...
            <Descriptions column={1} size="small" style={{ marginTop: 16 }}>
              ...
              <Descriptions.Item label="First Contact">
                {contact.first_contact_at ? dayjs(contact.first_contact_at).format('MMM D, YYYY') : 'N/A'}
              </Descriptions.Item>
          ...
          <Card
            title="Status & Notes"
            style={{ marginBottom: 16 }}
            extra={<Button type="text" icon={<EditOutlined />} onClick={() => openEditModal('status')}>Edit</Button>}
          >
            <Descriptions column={1} size="small">
                  ...
                  <Card>
                    ...
                    {contact.voter_status ? (
                      <>
                        <Descriptions column={2} size="small" bordered style={{ marginBottom: 24 }}>
                          <Descriptions.Item label="Voter Status">
                            <Tag color={
                              contact.voter_status === 'active' ? 'green' :
                              contact.voter_status === 'inactive' ? 'orange' : 'default'
                            }>
                              {formatLabel(contact.voter_status)}
                            </Tag>
                          </Descriptions.Item>
                        ...
                        <Title level={5} style={{ marginBottom: 16 }}>Election History</Title>
                        <Table
                          dataSource={[]}
                          columns={[
                            {
                            ...
                            },
                            {
                              ...
                              key: 'election_type',
                              render: (type: string) => (
                                <Tag color={type === 'general' ? 'blue' : type === 'primary' ? 'purple' : 'default'}>
                                  {formatLabel(type)}
                                </Tag>
                              ),
                            },
                            {
                              ...
                              render: (voted: boolean | null) => {
                                if (voted === true) return <CheckCircleOutlined style={{ color: '#52c41a', fontSize: 18 }} />;
                                if (voted === false) return <CloseCircleOutlined style={{ color: '#ff4d4f', fontSize: 18 }} />;
                            ...
                            },
                            {
                              ...
                              key: 'voting_method',
                              render: (method: string) => method ? formatLabel(method) : 'N/A',
                            },
                          ]}
                          rowKey="election_id"
                          pagination={false}
                          locale={{ emptyText: (
                            <Empty
                              ...
                              description={
                                <span>
                                  No voting history records<br />
                                  <Text type="secondary">Voting history will appear here once imported from voter files</Text>
                                </span>
                              }
                            />
                          )}}
                      ...
                      <Empty
                        ...
                        description={
                          <span>
                            Voter information not available<br />
                            <Text type="secondary">Add voter registration details in the Voter Information section</Text>
                          </span>
                        }
                      >
                    ...
                    <Card type="inner" title="Engagement Score" style={{ marginBottom: 16 }}>
                      <Row gutter={24} align="middle">
                        <Col span={8}>
                          <Statistic
                            title="Overall Score"
                            value={contact.message_count > 0 ? Math.min(100, contact.message_count * 10 + 20) : 0}
                            suffix="/ 100"
                            valueStyle={{ color: contact.message_count > 5 ? '#52c41a' : contact.message_count > 0 ? '#faad14' : '#d9d9d9' }}
                          />
                        </Col>
                        <Col span={16}>
                          <Text type="secondary">
                            Based on message frequency, tone patterns, and interaction history.
                        ...
                        description={
                          <div>
                            <Paragraph style={{ marginBottom: 8 }}>
                              This section will provide AI-generated recommendations to encourage participation from this contact, including:
                            </Paragraph>
                            <ul style={{ marginBottom: 0, paddingLeft: 20 }}>
                              <li>Personalized outreach suggestions based on communication tone</li>
                              <li>Optimal contact timing based on historical engagement</li>
                              <li>Issue-based talking points aligned with expressed interests</li>
                              <li>Event invitations relevant to their location and demographics</li>
                      ...
                      <Descriptions column={1} size="small">
                          ...
                          {contact.dominant_tones && contact.dominant_tones.length > 0 ? (
                            <Space wrap>
                              {contact.dominant_tones.map((tone) => (
                                <Tag key={tone} color={getToneColor(tone)}>{tone}</Tag>
                              ))}
                            </Space>
                          ) : (
                        ...
                        <Descriptions.Item label="Contact Completeness">
                          {(() => {
                            const fields = [
                              ...
                              contact.voter_status, contact.party_affiliation,
                              contact.occupation, contact.date_of_birth
                            ];
                            ...
                            return (
                              <Space>
                                ...
                                <Text type="secondary">({filled} of {fields.length} key fields)</Text>
                              </Space>
                            );
                          })()}
                        </Descriptions.Item>
                        <Descriptions.Item label="Last Activity">
                          {contact.last_contact_at
                            ? `${dayjs(contact.last_contact_at).format('MMM D, YYYY')} (${dayjs(contact.last_contact_at).fromNow()})`
                      ...
                      <Space direction="vertical" style={{ width: '100%' }}>
                        {!contact.phone && (
                          <Alert
                            ...
                            showIcon
                            action={
                              <Button size="small" onClick={() => openEditModal('contact')}>
                                Add
                              </Button>
                            }
                          />
                        ...
                        {!contact.address && (
                          <Alert
                            ...
                            showIcon
                            action={
                              <Button size="small" onClick={() => openEditModal('location')}>
                                Add
                              </Button>
                            }
                          />
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="preferred_language" label="Preferred Language">
                <Input placeholder="en, es, etc." />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="communication_preference" label="Contact Preference">
                <Select options={COMMUNICATION_PREF_OPTIONS} allowClear placeholder="Select preference" />
              </Form.Item>
            </Col>
          </Row>
        </Form>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="gender" label="Gender">
                <Select options={GENDER_OPTIONS} allowClear placeholder="Select gender" />
              </Form.Item>
            </Col>
            <Col span={12}>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="education_level" label="Education Level">
                <Select options={EDUCATION_OPTIONS} allowClear placeholder="Select level" />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="income_bracket" label="Income Bracket">
                <Select options={INCOME_OPTIONS} allowClear placeholder="Select range" />
              </Form.Item>
            </Col>
          </Row>
          <Form.Item name="homeowner_status" label="Homeowner Status">
            <Select options={HOMEOWNER_OPTIONS} allowClear placeholder="Select status" />
          </Form.Item>
        </Form>
      </Modal>
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item name="voter_status" label="Voter Status">
            <Select options={VOTER_STATUS_OPTIONS} allowClear placeholder="Select status" />
          </Form.Item>
            ...
            {({ getFieldValue }) =>
              !getFieldValue('is_active') && (
                <Form.Item name="inactive_reason" label="Reason for Inactivity">
                  <Select options={INACTIVE_REASON_OPTIONS} allowClear placeholder="Select reason" />
                </Form.Item>
              )
            }
          </Form.Item>
