...
import { useContactsQuery, useCreateContactMutation, ContactFilters } from '../services/contactsService';
    ...
    const columns: ColumnsType<Contact> = [
        {
            ...
            ellipsis: true,
            render: (_, record) => (
                <a onClick={() => navigate(`/contacts/${record.id}`)}>{record.name || record.email}</a>
            ),
        },
        {
            title: 'Messages',
            ...
            align: 'center',
            sorter: true,
            render: (count: number) => <Tag color={count > 10 ? 'blue' : 'default'}>{count}</Tag>,
        },
        {
            title: 'Tones',
            ...
            render: (tones: ToneLabel[]) => (
                <Space wrap size={[0, 4]}>
                    ...
                    {(!tones || tones.length === 0) && <Tag color='default'>-</Tag>}
                </Space>
            ),
        },
        {
    ...
    return (
        <div>
            ...
            <Row gutter={16} style={{ marginBottom: 24 }}>
                <Col span={8}>
                    <Card>
                        <Statistic title='Total Contacts' value={stats.total} prefix={<UserOutlined />} />
                    </Card>
                </Col>
                <Col span={8}>
                    <Card>
                        <Statistic title='With Messages' value={stats.withMessages} prefix={<MailOutlined />} />
                    </Card>
                </Col>
                <Col span={8}>
                    <Card>
                        <Statistic title='With Tones' value={stats.withTones} valueStyle={{ color: '#1890ff' }} />
                    </Card>
                </Col>
            </Row>
